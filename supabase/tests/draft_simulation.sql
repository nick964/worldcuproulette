...
-- no data behind. Impersonates users by setting the JWT `sub` claim the same
...
begin
  -- Arrange: a pool and 7 members (inserted directly; editor bypasses RLS).
  insert into pools (id, name, owner_id, invite_code, status)
  values (v_pool, 'Sim Pool', v_owner, 'sim-' || substr(v_pool::text, 1, 8), 'open');

  for v_i in 1..7 loop
    v_user := 'sim_user_' || v_i;
    insert into pool_members (pool_id, user_id, role)
      values (v_pool, v_user, case when v_i = 1 then 'owner' else 'member' end);
  end loop;
